...
local estados = {
    {x=0, y=0, xScale=3, yScale=2, anchorX=0.15, anchorY=0.10, time = 2000, onComplete = setControlador},
    {x=0, y=0, xScale=2.9, yScale=1.4, anchorX=0.5, anchorY=0.07, time = 3000, onComplete = setControlador},
...
function backHistoria2( self, event )
    if event.phase == "ended" then 
    print("touch")
...
function scene:create( event )
    ...
    background = display.newImageRect(sceneGroup, "Assets/Frames/65.jpg", cw, ch )
    background.x = cw/2
    background.y = ch/2
    ...
    frame1 = graphics.newImageSheet( "Assets/Frames/frame3.png", options )
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        backScene.touch = backHistoria2
        backScene:addEventListener( "touch", backScene )

        nextScene.touch = irEscena4
        nextScene:addEventListener( "touch", nextScene )

        backgroundMusic = audio.loadStream("pagina3.mp3")
