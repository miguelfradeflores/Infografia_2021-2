...
local estados = {
    {x=0, y=0, xScale=2.5, yScale=2.4, anchorX=0.1, anchorY=0.1, time = 2000, onComplete = setControlador},
    {x=0, y=0, xScale=2.9, yScale=2.9, anchorX=0.13, anchorY=0.46, time = 3000, onComplete = setControlador},
...
function backHistoria3( self, event )
    if event.phase == "ended" then 
    print("touch")
...
function scene:create( event )
    ...
    background = display.newImageRect(sceneGroup, "Assets/Frames/78.jpg", cw, ch )
    background.x = cw/2
    background.y = ch/2
    ...
    frame1 = graphics.newImageSheet( "Assets/Frames/frame4.png", options )
...
function scene:show( event )
    ...
    elseif ( phase == "did" ) then
        ...
        backScene.touch = backHistoria3
        backScene:addEventListener( "touch", backScene )

        nextScene.touch = irEscena5
        nextScene:addEventListener( "touch", nextScene )

        backgroundMusic = audio.loadStream("pagina4.mp3")
