...
function setupPlayer()
	...
	local playerSheet = graphics.newImageSheet( "Images/player.png", options )
	local sequenceData = {
...
function firePlayerBullet()
	if(canFireBullet == true)then
		local tempBullet = display.newImage("Images/laser.png", player.x, player.y - playerHeight/ 2)
...
function setupInvaders()
	...
	for i = 1, gameData.rowsOfInvaders do
		for j = 1, numberOfInvaders do
			local tempInvader = display.newImage("Images/invader1.png",xPositionStart + ((invaderSize+10)*(j-1)), i * 46 )
...
function fireInvaderBullet()
	if(#invadersWhoCanFire >0) then
		...
		local tempInvaderBullet = display.newImage("Images/laser.png", randomInvader.x , randomInvader.y + invaderSize/2)
